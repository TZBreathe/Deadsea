...
for i=1:length(Br)
    ...
    MaxCap{i}=CapList{i}(1);
    y{i}=CapList{i}(:)/MaxCap{i};
...
plt=[1 14 15 19 20 21 22 23 24];

for i=1:length(plt)
 
Idx_est=50;
EstMdl(i)=estimate(Mdl,yS{plt(i)}((end-Idx_est):end));    
yf{plt(i)}=forecast(EstMdl(i),200,yS{plt(i)}((end-Idx_est):end-3));    
cycl{i}=1:length(y{plt(i)});  
cycl2{i}=cycl{i}(end-3):(cycl{i}(end)+196);    
