...
#autoarima test
def run_arimaxx_auto(x1,x2,prediction_len):
    from pmdarima.arima import auto_arima
    train_size = int(len(x1) * 0.75)
    train, test = x1[0:train_size], x1[train_size:]
    ex=np.vstack((x2[0:train_size],np.zeros(train_size))).T
    ex_p=np.vstack((x2[train_size:len(x2)],np.zeros(len(x2)-train_size))).T
    model_fit=auto_arima(train,X=ex,supress_warnings=True,maxiter=50)
    yhat=model_fit.predict(n_periods=len(ex_p),X=ex_p)
    xhat=np.arange(len(train),len(x1)+prediction_len)
    return np.vstack((xhat,yhat))
