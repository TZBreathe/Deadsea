...
pred_arima=py.arima_functions.run_arima(X,cycle,pred_len,best_order);
...
cycles=[1:length(yS)+double(pred_len)]';
cycle=py.numpy.array(cycles);
pred_arima=py.arima_functions.run_arima_auto2(X,cycle,pred_len);
pred_arima=double(pred_arima);
